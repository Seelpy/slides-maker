...
function useDragObject(
  ...
  useEffect(() => {
    ...
    if (!area)
      throw new Error(
        "Wrong slide object structure! Object doesn't have a parent-area",
      )
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      const deltaHeight =
        e.clientY - coords.current.startMouse.y + coords.current.startOffset.y
      const deltaWidth =
        e.clientX - coords.current.startMouse.x + coords.current.startOffset.x
