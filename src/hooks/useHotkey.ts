...
function useHotkey(hotkeys: string[], callback: (event: KeyboardEvent) => void) {
  // Разделяет строку по первому '+' после конца слова
  const hotkeyArrays = hotkeys.map((hotkey) => Array.from(new Set(
    hotkey.toLocaleLowerCase().replace(/\s+/g, "").split(/\b\+/g)
  )))
  ...
  const isSameHotkeys = (event: KeyboardEvent): boolean => {
    ...
    for (let i = 0; i < hotkeyArrays.length; i++) {
      ...
      if (hotkeyArrays[i].length === activeKeys.length &&
