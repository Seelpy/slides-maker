  ...
  selectionRef: React.MutableRefObject<HTMLDivElement | null>) 
{
  ...
  const coords = useRef<{
    ...
    selection: { position: {x: -1e5, y: -1e5}, size: {width: 0, height: 0} },
  })

  const isObjectInArea = (obj: SlideObject, area: {position: Position, size: Size}): boolean => {
    return (
  ...
  useEffect(() => {
    ...
    if (selectionRef.current) {
      if (isSelectingArea) {
        ...
        coords.current.selection = {position: {x: -1e5, y: -1e5}, size: {width: 0, height: 0}};
      }
      else {
