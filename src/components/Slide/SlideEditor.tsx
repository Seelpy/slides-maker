...
const SlideEditor = () => {
  ...
  const unselectObjects = (event: React.MouseEvent) => {
    if (event.target === editorAreaRef.current || event.target === slideEditorRef.current) {
      referencedActiveSlide!.slide.map(obj => updateSlide({slide: referencedActiveSlide!, oldSlideObject: obj, newSlideObject: {...obj, selected: false}}));
    }
  }

  return (
    <div className={styles.editorArea} ref={editorAreaRef} onMouseLeave={() => setDragObjects(false)} onClick={(e) => unselectObjects(e)}>
