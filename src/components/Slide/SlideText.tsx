...
const SlideText = (props: TextProps) => {
  ...
  const lastHistoryOperation = useAppSelector(
    (state) => state.historyReducer.lastHistoryOperation,
  )
  ...
  useEffect(() => {
    if (textRef.current && lastHistoryOperation === undefined) {
      ...
      if (
        textSize.width !== data.size.width ||
        textSize.height !== data.size.height
      ) {
        setLastOperationType(HistoryOperation.update)
        updateSlide({
          slideId: props.slide.id,
          oldSlideObject: data,
          newSlideObject: { ...data, size: textSize },
        })
      }
    }
  }, [data])
